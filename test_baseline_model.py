...
class TestBaselineModel:
    ...
    def test_training_history_recorded(self, sample_training_data):
        ...
        # يجب أن يكون هناك سجلات (كل 100 دورة)
    ...
    def test_model_convergence(self, sample_training_data):
        """اختبار أن النموذج يتقارب (الخطأ يقل مع التدريب)"""
        model = BaselineModel(learning_rate=0.001, epochs=500)
        model.fit(
            train_data=sample_training_data,
            feature_keys=["age", "tumor_volume"],
            target_key="dose_gy"
        ...
        # التحقق من أن الخطأ قل مع التدريب
        if len(model.training_history) >= 2:
            first_mse = model.training_history[0]["mse"]
            last_mse = model.training_history[-1]["mse"]
    ...
    def test_predict_consistency(self, trained_model, sample_test_data):
        """اختبار أن التنبؤات متسقة (نفس المدخلات = نفس المخرجات)"""
